...
def _normalise(value):
    """Make equal things look equal: 12.0 and 12 are the same event id."""
...
def idempotency_key(run_id: str, step_seq: int, tool_name: str, args: dict) -> str:
    ...
    A resumed run rebuilds the same step numbers from the database, so the key it computes for a
    call it already made is the key already stored against that call's result.
    """
...
def notification_dedupe_key(roll_no: str, message: str, day: date) -> str:
    """The same message to the same student on the same day is one message.

    This one does not depend on the run at all, so it holds even when a different run, a different
    worker or a retry produces the same text. Whitespace is collapsed: a model that re-wraps a line
    has not written a new message.
    """
