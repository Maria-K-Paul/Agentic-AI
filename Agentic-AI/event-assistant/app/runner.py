...
The important property is that nothing here keeps state in memory that it could not rebuild from the
database. A worker that dies at any point leaves a run another worker can pick up and finish, and
`rebuild` is what turns the rows back into the conversation the model had been having.
"""
import time
...
def rebuild(store: RunStore, thread_id: str, run_id: str):
    ...
    A pending call is the interesting case: the model asked for it and we do not know whether it ran.
    The run redoes it, and the idempotency key decides whether that is a real call or a replay.
    """
...
def execute_run(claimed: Claimed, *, store: RunStore, db: EventsDb, providers: dict,
    ...
    def between_steps() -> str | None:
        """The only place a run is allowed to stop: between steps, never inside a tool."""
    ...
    while True:
        for step_seq, call in pending:
            ...
            if on_step:
                on_step({"agent": "supervisor", "run_id": run_id, "step": step_seq, "kind": "tool",
                         "tool": call["name"], "args": call["args"], "result": result, "ok": ok, "ms": ms})
        pending = []
        ...
        if on_step:
            on_step({"agent": "supervisor", "run_id": run_id, "step": seq, "kind": "model",
