"""A worker: reap the dead, claim a run, execute it, record how it ended.

Workers are interchangeable and stateless. Run as many as you like; `claim_next` uses SKIP LOCKED,
so they take different runs without coordinating.
"""
import logging
import os
import socket
...
class Worker:
    def __init__(self, store: RunStore, db: EventsDb, providers: dict, worker_id: str | None = None,
    ...
    def run_once(self) -> tuple[str, str] | None:
        """Take one run to an end, or return None when the queue has nothing for us.

        Every failure path ends in a row change, because a run left 'running' by a worker that gave
        up quietly would sit there until its lease expired.
        """
